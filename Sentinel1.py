...
class Sentinel1(Sensor):
    ...
    def validateUserInputs(self):
        ...
        if (not self.tiff) and (self.safe):
            ...
            if dirname.endswith('.zip'):
                pattern = os.path.join(before_name,'measurement', swathid) + '*.tiff'
                zf = zipfile.ZipFile(dirname, 'r')
                match = fnmatch.filter(zf.namelist(), pattern)
                zf.close()
        ...
        if self.safe:
            if dirname.endswith('.zip'):
                pattern=os.path.join(before_name,'manifest.safe')
                zf = zipfile.ZipFile(dirname, 'r')
                match = fnmatch.filter(zf.namelist(), pattern)
                zf.close()
    ...
    def grab_from_xml2(self, path):  ### 检查是否存在值！
        target_tag = self._xml_root.find(path)
        return target_tag
...
#        height = self.product.imageGenerationParameters.sarProcessingInformation._satelliteHeight
    ...
    def extractOrbitFromAnnotation(self):
        ...
        if temp_pluse is None or temp_pluse.text is None:
            newOrb = frameOrbit
            print ("DJ1 Orbits Need NOT Extend!")
        else:
    ...
    def extractImage(self):
        ...
        for ii in range(lgth):
            ...
            data.tofile(fid)
        #print ("SLC data: width， lgth", width, lgth)
        fid.close()
        src = None
        band = None
